  ...
  <scroll-view scroll-y class="viewport" id="scroller" @scrolltolower="onScrolltolower">
    <template v-if="orderList">
      ...
      <view class="overview" :style="{ paddingTop: safeAreaInsets!.top + 20 + 'px' }">
        ...
        <template v-else>
          <!-- 订单状态文字 -->
          <view class="status"> {{ orderStateList[orderList.orderState].text }} </view>
          <view class="button-group">
            <navigator
              class="button"
              :url="`/pagesOrder/creat/creat?orderId=${query.id}`"
              hover-class="none"
            >
              再次购买
            </navigator>
      ...
      <view class="goods">
        <view class="item">
          ...
          <!-- 待评价状态:展示按钮 -->
          <view class="action" v-if="orderList.orderState === OrderState.DaiPingJia">
        ...
        <view class="total">
          <view class="row">
            ...
            <view class="symbol">{{ orderList.totalMoney }}</view>
          </view>
          <view class="row">
            <view class="text">运费: </view>
            <view class="symbol">{{ orderList.postFee }}</view>
          </view>
          <view class="row">
            <view class="text">应付金额: </view>
            <view class="symbol primary">{{ orderList.payMoney }}</view>
          </view>
        </view>
      </view>
      ...
      <view class="detail">
        ...
        <view class="row">
          ...
          <view class="item">下单时间: {{ orderList.createTime }}</view>
        </view>
      </view>
      ...
      <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
        ...
        <template v-else>
          ...
          <!-- 待评价状态: 展示去评价 -->
          <view class="button" v-if="orderList.orderState === OrderState.DaiPingJia"> 去评价 </view>
          <!-- 待评价/已完成/已取消 状态: 展示删除订单 -->
          <view
            class="button delete"
            v-if="orderList.orderState >= OrderState.DaiPingJia"
            @tap="onDeleteOrder"
          >
            删除订单
          </view>
